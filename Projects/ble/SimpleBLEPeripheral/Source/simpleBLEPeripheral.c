...
static void simpleProfileChangeCB( uint8 paramID )
{
  ...
  switch( paramID )
  {
    case SIMPLEPROFILE_CHAR1:
      ...
      #endif // (defined HAL_LCD) && (HAL_LCD == TRUE)
      //¸ù¾ÝCHAR1µÄÖµ¸Ä±äLED×´Ì¬
      if(newValue == 0x31)
      {
          HalLedSet(HAL_LED_2,HAL_LED_MODE_ON); //¿ªLED2
      }
      else
      {
          HalLedSet(HAL_LED_2,HAL_LED_MODE_OFF); //¹ØLED2
      }
...
static void NpiSerialCallback(uint8 port,uint8 events)
{
    ...
    if(events & HAL_UART_RX_TIMEOUT)    //´®¿ÚÓÐÊý¾Ý
    {
        ...
        if(numBytes)
        {
            //´Ó´®¿Ú»º³åÇø¶Á³önumBytes×Ö½ÚÊý¾Ý
          NPI_ReadTransport(buf,numBytes);
          //°Ñ½ÓÊÕµ½´®¿ÚÊý¾ÝÔÙ´òÓ¡³öÀ´
...
{
    if((events & HAL_UART_TX_EMPTY)||( events & HAL_UART_TX_FULL ))  // ·¢ËÍÇøÂú»òÕß¿Õ
    {
        return;
    }
    ...
    if(usRxBufLen)
    {
        ...
        uint16 readLen = HalUARTRead(HAL_UART_PORT_0, &SerialRxBuf[RxIndex], usRxBufLen);   // ¶ÁÈ¡Êý¾Ýµ½»º³åÇø
 //       NPI_WriteTransport(readLen,usRxBufLen);
        RxIndex += readLen;
        readLen %= 128;
        osal_start_timerEx(simpleBLEPeripheral_TaskID, UART_EVENT, 5);  // Æô¶¯¶¨Ê±Æ÷
    }
}
...
static void Serial_Init(void)
{
    ...
    SerialCfg.baudRate = HAL_UART_BR_115200;    // ²¨ÌØÂÊ
    SerialCfg.flowControl = HAL_UART_FLOW_OFF;  // Á÷¿ØÖÆ

    SerialCfg.callBackFunc = SerialCb;          // »Øµ÷º¯Êý
    SerialCfg.intEnable    = TRUE;
    SerialCfg.configured   = TRUE;
    HalLcdWriteString( "Open Uart0", HAL_LCD_LINE_5 );    // ÔÚµÚ5ÐÐÏÔÊ¾Æô¶¯ÐÅÏ¢
